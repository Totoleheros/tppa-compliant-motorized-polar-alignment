#!/usr/bin/env python3
"""
PolarAlign Controller v15.03g-V4 – Desktop GUI for the ESP32 Polar Alignment System
...
Run:      python3 PolarAlignGUI_v15_03g_V4.py
"""
...
class App:
    ...
    def __init__(self, root, profile_name):
        ...
        self.root.title(f"PolarAlign Controller v15.03g-V4  —  {profile_name}")
    ...
    def _build(self):
        # — Top: Connection bar — (compact pady)
        cf = tk.LabelFrame(self.root, text="Connection", bg=BG, fg=TXT_DIM,
        ...
        sf = tk.Frame(self.root, bg=BORDER, padx=12, pady=6)
        sf.pack(fill="x", padx=10, pady=4)

        # Row 1: state | AZM pos | ALT pos                                MPU
        row1 = tk.Frame(sf, bg=BG)
        row1.pack(fill="x")
        self.st_lbl = tk.Label(row1, text="—", font=("Helvetica", 17, "bold"),
                                fg="#666", bg=BG, width=6, anchor="w")
        self.st_lbl.pack(side="left", padx=(0, 16))
        self.azm_lbl = tk.Label(row1, text="AZM    0.000°    (   0.0')",
                                 font=(MONO, 15), fg=CYAN, bg=BG)
        self.azm_lbl.pack(side="left", padx=(0, 24))
        self.alt_lbl = tk.Label(row1, text="ALT    0.000°    (   0.0')",
                                 font=(MONO, 15), fg=AMBER, bg=BG)
        self.alt_lbl.pack(side="left")
        self.mpu_lbl = tk.Label(row1, text="MPU  —",
                                 font=(MONO, 13), fg="#aaaaaa", bg=BG)
        self.mpu_lbl.pack(side="right", padx=(16, 0))

        # Row 2: AZM learning  |  ALT learning
        row2 = tk.Frame(sf, bg=BG)
        row2.pack(fill="x", pady=(4, 0))

        # AZM learning block (blue tones)
        azm_lrn = tk.Frame(row2, bg=BG)
        azm_lrn.pack(side="left", padx=(0, 24))
        tk.Label(azm_lrn, text="AZM", font=(MONO, 10, "bold"),
                 fg=CYAN, bg=BG).pack(side="left")
        tk.Label(azm_lrn, text=" ratio:", font=(MONO, 10),
                 fg=TXT_DIM, bg=BG).pack(side="left")
        self._lbl_azm_ratio = tk.Label(azm_lrn, text="—",
                                        font=(MONO, 10), fg="#4CAF50", bg=BG,
                                        width=18, anchor="w")
        self._lbl_azm_ratio.pack(side="left", padx=(4, 8))
        tk.Label(azm_lrn, text="last:", font=(MONO, 10),
                 fg=TXT_DIM, bg=BG).pack(side="left")
        self._lbl_azm_upd = tk.Label(azm_lrn, text="—",
        ...
        # ALT learning block (orange tones)
        alt_lrn = tk.Frame(row2, bg=BG)
        alt_lrn.pack(side="left")
        tk.Label(alt_lrn, text="ALT", font=(MONO, 10, "bold"),
                 fg=AMBER, bg=BG).pack(side="left")
        tk.Label(alt_lrn, text=" ratio:", font=(MONO, 10),
                 fg=TXT_DIM, bg=BG).pack(side="left")
        self._lbl_alt_ratio = tk.Label(alt_lrn, text="—",
                                        font=(MONO, 10), fg="#4CAF50", bg=BG,
                                        width=18, anchor="w")
        self._lbl_alt_ratio.pack(side="left", padx=(4, 8))
        tk.Label(alt_lrn, text="err:", font=(MONO, 10),
                 fg=TXT_DIM, bg=BG).pack(side="left")
        self._lbl_alt_err = tk.Label(alt_lrn, text="—",
                                      font=(MONO, 10), fg="#4CAF50", bg=BG,
                                      width=8, anchor="w")
        self._lbl_alt_err.pack(side="left", padx=(4, 8))
        tk.Label(alt_lrn, text="act/tgt:", font=(MONO, 10),
                 fg=TXT_DIM, bg=BG).pack(side="left")
        self._lbl_alt_acttgt = tk.Label(alt_lrn, text="—",
                                         font=(MONO, 10), fg="#aaaaaa", bg=BG,
                                         width=18, anchor="w")
    ...
    def _build_ctrl(self, nb):
        # v15.03g-V4: tighter outer padding to free vertical room for bottom row
        tab = tk.Frame(nb, bg=BG, padx=10, pady=8)
        tab.configure(bg=BG)
        ...
        def jog(parent, label, sublabel, color, delta, axis):
            make_button(parent, f"{label}\n{sublabel}", bg=color, fg="white",
                        font=("Helvetica", 10, "bold"), padx=4, pady=3,
                        command=lambda: self._jog(axis, delta)
                        ).pack(side="left", padx=1, pady=2)

        # ── AZM ──────────────────────────────────────────────────────────
        azm_outer = tk.Frame(tab, bg=BG, highlightbackground=BORDER, highlightthickness=1)
        azm_outer.pack(fill="x", pady=(0, 6))
        tk.Label(azm_outer, text="  Azimuth (AZM)  —  RELATIVE moves",
                 bg=BG, fg=CYAN, font=("Helvetica", 11, "bold"),
                 anchor="w").pack(fill="x", padx=8, pady=(4, 1))
        azm_lf = tk.Frame(azm_outer, bg=BG, padx=10, pady=4)
        azm_lf.pack(fill="x")

        dh = tk.Frame(azm_lf, bg=BG)
        dh.pack(fill="x")
        tk.Label(dh, text="← WEST", bg=BG, fg=BTN_WEST,
                 font=("Helvetica", 12, "bold")).pack(side="left")
        tk.Label(dh, text="EAST →",   bg=BG, fg=BTN_EAST,
                 font=("Helvetica", 12, "bold")).pack(side="right")

        azm_row = tk.Frame(azm_lf, bg=BG)
        azm_row.pack()
        # WEST: −1°  |  −30' −10' −5' −1'  |  −30" −10"
        ...
        ga.pack(pady=(6, 0))
        tk.Label(ga, text="Go to (°)  [ABS]:", bg=BG,
                 fg=TXT_DIM, font=("Helvetica", 12)).pack(side="left")
        ...
        for deg, lbl, eq in JOG_ARCSEC_STEPS:
            jog(dn_row, f"−{lbl}", eq, BTN_DOWN, -deg, "ALT")
        tk.Label(alt_lf, text="▼  DOWN", bg=BG, fg=BTN_DOWN,
                 font=("Helvetica", 12, "bold")).pack(anchor="w")

        galt = tk.Frame(alt_lf, bg=BG)
        galt.pack(pady=(6, 0))
        tk.Label(galt, text="Go to (°)  [ABS]:", bg=BG,
                 fg=TXT_DIM, font=("Helvetica", 12)).pack(side="left")
        ...
        # ── BOTTOM: System Commands only (Learning Monitor moved to top bar) ──
        # v15.03g-V4: full-width System Commands so all 4 buttons stay visible
        sl_outer = tk.Frame(tab, bg=BG, highlightbackground=BORDER, highlightthickness=1)
        sl_outer.pack(fill="x", pady=(2, 0))
        tk.Label(sl_outer, text="  System Commands", bg=BG, fg=TXT_DIM,
                 font=("Helvetica", 10), anchor="w").pack(fill="x", padx=8, pady=(3, 1))
        sl = tk.Frame(sl_outer, bg=BG, pady=6, padx=10)
        sl.pack(fill="x")
