...
void loop() {
  ...
  // Short press (<1s) during TPPA session  → dump DIAG to serial (TPPA ignores unknown lines)
  // Short press when idle                  → start homing
  // Long press  (>1s) always               → start homing
  if (digitalRead(PIN_BUTTON_HOME) == LOW) {
    delay(50);
    if (digitalRead(PIN_BUTTON_HOME) == LOW) {
      unsigned long pressStart = millis();
      while (digitalRead(PIN_BUTTON_HOME) == LOW) delay(10);
      bool longPress = (millis() - pressStart) > 1000;
      if (!longPress && homingDone) {   // any time after homing: short=DIAG, long=HOME
        printDiagnostic();
      } else if (!isMoving || longPress) {
        startHoming();
      }
      return;
    }
