 ...
 * Version : 14.69 - HOMING GUARD + AZM BACKLASH COMPENSATION
 *
 * Changelog vs 14.68:
 *  - SAFETY: Added homingDone guard. TPPA jog commands ($J=) are silently
 *    ignored (ok replied but no movement) until HOME/$H has been executed.
 *    Without homing, posDegALT starts at 0.0 regardless of physical position,
 *    causing ALT travel limits to clamp incorrectly and TPPA to loop forever.
 *  - FIX: AZM backlash compensation for harmonic drive lost motion.
 *    On direction reversal, extra "dead" steps are injected at the start
 *    of the move to eat through the flex spline's elastic deformation zone.
 *    These steps move the motor but do NOT update the reported position.
 *    Configurable via AZM_BACKLASH_DEG (default 0.033° = 2 arcmin).
 *  - Direct serial commands (ALT:, AZM:) remain accessible without homing.
 *  - Boot banner warns prominently that HOME is required.
 *
 * Inherited from 14.68:
...
void startHoming() {
  ...
  homingDone = true;
  learningRequestedDelta = 0;
  Serial.println("MSG: Homing OK. TPPA jogs now enabled."); sendStatus(); Serial.println();
}

/* ───── DIAGNOSTIC ───── */
void printDiagnostic() {
  Serial.println("\n--- SYSTEM DIAGNOSTIC (v14.69 homing-guard + AZM backlash) ---");
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=======================================================");
  Serial.println("  BOOT: POLAR ALIGNMENT CONTROLLER V14.69 (ESP32)");
  ...
  Serial.println("  *** HOME REQUIRED BEFORE TPPA — run HOME or $H ***");
  Serial.println("=======================================================\n");
  ...
  if (digitalRead(PIN_HOME_SENSOR) == LOW) {
    ...
    Serial.println("MSG: System READY. TPPA jogs LOCKED until HOME is executed.");
