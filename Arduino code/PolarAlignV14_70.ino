 ...
 * Version : 14.70 - DTR-PROOF HOMING PERSISTENCE
 *
 * Changelog vs 14.69:
 *  - FIX: Homing state now survives DTR-triggered reboots (GUI→TPPA switch).
 *    mpuOffset + magic word saved to EEPROM on homing completion.
 *    On boot, if magic is valid, firmware restores mpuOffset, reads MPU to
 *    reconstruct posDegALT, and sets homingDone=true automatically.
 *    AZM position resets to 0 on reboot (no sensor — use AZM:ZERO if needed).
 *  - Sanity check: if restored ALT position is outside travel limits,
 *    homing state is invalidated (physical displacement while powered off).
 *
 * Inherited from 14.69:
 *  - Homing guard (TPPA jogs blocked until HOME)
 *  - AZM backlash compensation (harmonic drive lost motion)
 *  - MPU observe-only (ML learning, no corrections)
...
/* ───── EEPROM LAYOUT ───── */
constexpr int EEPROM_SIZE = 16;              // total bytes reserved
constexpr int EEPROM_ADDR_RATIO    = 0;      // float: activeStepsPerDegALT
constexpr int EEPROM_ADDR_MPU_OFF  = 4;      // float: mpuOffset (tare)
constexpr int EEPROM_ADDR_MAGIC    = 8;      // uint32_t: homing validity flag
...
/* ───── AZM BACKLASH COMPENSATION (harmonic drive lost motion) ───── */
constexpr float AZM_BACKLASH_DEG = 0.033f;  // ~2 arcmin — adjust to your harmonic drive
...
int8_t lastAzmDir = 0;  // +1 = positive, -1 = negative, 0 = unknown
...
struct {
  ...
  long totalSteps;
  long stepsDone;
  long backlashSteps;  // dead steps at start (no position update)
} mot = {false};
...
void tickMotion() {
  ...
  /* Position update — skip during backlash dead steps */
...
void startHoming() {
  ...
  /* ── Persist homing state so it survives DTR reboots ── */
  EEPROM.put(EEPROM_ADDR_MPU_OFF, mpuOffset);
  uint32_t magic = HOMING_MAGIC;
  EEPROM.put(EEPROM_ADDR_MAGIC, magic);
  EEPROM.commit();

  Serial.println("MSG: Homing OK. TPPA jogs now enabled (state saved to EEPROM)."); sendStatus(); Serial.println();
}

/* ───── DIAGNOSTIC ───── */
void printDiagnostic() {
  Serial.println("\n--- SYSTEM DIAGNOSTIC (v14.70 DTR-proof homing persistence) ---");
  ...
  Serial.print("homingDone            : "); Serial.println(homingDone ? "YES — TPPA jogs enabled" : "NO — *** TPPA JOGS BLOCKED ***");
  Serial.print("AZM backlash comp     : "); Serial.print(AZM_BACKLASH_DEG * 60.0f, 1); Serial.print("' ("); Serial.print(AZM_BACKLASH_DEG, 4); Serial.println(" deg)");
  Serial.print("lastAzmDir            : "); Serial.println(lastAzmDir == 1 ? "+1 (positive)" : lastAzmDir == -1 ? "-1 (negative)" : "0 (unknown)");
  ...
  float stored = 0.0f; EEPROM.get(EEPROM_ADDR_RATIO, stored);
  Serial.println(stored, 3);
  uint32_t storedMagic = 0; EEPROM.get(EEPROM_ADDR_MAGIC, storedMagic);
  Serial.print("EEPROM Homing State   : ");
  Serial.println(storedMagic == HOMING_MAGIC ? "SAVED (persists across reboot)" : "NOT SAVED");
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=======================================================");
  Serial.println("  BOOT: POLAR ALIGNMENT CONTROLLER V14.70 (ESP32)");
  ...
  Serial.print("  AZM backlash comp: "); Serial.print(AZM_BACKLASH_DEG * 60.0f, 1); Serial.println(" arcmin");
  ...
  if (digitalRead(PIN_HOME_SENSOR) == LOW) {
    ...
    /* ── Try to restore homing state from EEPROM (survives DTR reboot) ── */
    ...
    if (savedMagic == HOMING_MAGIC && mpuAvailable) {
      ...
      float rawAngle = readMPUAngleY();
      if (rawAngle > -900.0f) {   // MPU read OK
        ...
        if (restoredALT >= (ALT_LIMIT_NEG - 0.5f) &&
          ...
          Serial.print("  posDegALT = "); Serial.print(posDegALT, 3);
          Serial.println(" deg");
          Serial.println("MSG: TPPA jogs enabled (no re-homing needed).");
        } else {
          Serial.print("MSG: EEPROM homing data invalid — restored ALT = ");
          Serial.print(restoredALT, 3);
          Serial.println(" deg (outside travel limits). Please re-HOME.");
          Serial.println("MSG: TPPA jogs LOCKED until HOME is executed.");
        }
      } else {
        Serial.println("MSG: MPU read failed — cannot restore homing state.");
        Serial.println("MSG: TPPA jogs LOCKED until HOME is executed.");
      }
    } else {
      Serial.println("MSG: No saved homing state found.");
      Serial.println("MSG: TPPA jogs LOCKED until HOME is executed.");
      Serial.println("MSG: Send HOME or $H, or press the Home button.");
    }
  }
  Serial.println("-------------------------------------------------------\n");
}
