 ...
 * Version : 14.64 - LINEAR SCALE REPORTING + AZM CRUISE SPEED REDUCTION
 *
 * Changelog vs 14.63:
 *  - sendStatus() now uses LINEAR SCALING instead of a hard cap during feedback.
 *    Reported position progresses smoothly through the entire move but compressed
 *    so it never reaches the target. Brief ~1.5s pause at ~90% looks natural.
 *  - FEEDBACK_REPORT_MARGIN reduced from 0.50° to 0.10° (no more mid-course freeze).
 *  - AZM cruise speed halved (120µs → 240µs) to reduce mechanical stress on
 *    the Harmonic Drive under heavy payload. ALT cruise unchanged.
 *  - RMS_CURRENT_ALT = 300mA (thermal fix from v14.63 retained).
 *
...
constexpr unsigned long RAMP_CRUISE_AZM_US = 240;   // Halved for HD safety under load
constexpr unsigned long RAMP_CRUISE_ALT_US = 120;   // ALT unchanged (high ratio = already slow)
...
void sendStatus() {
  ...
  /*  v14.64: LINEAR SCALE REPORTING
   *  During inFeedbackCycle, compress the reported position so it never
   *  reaches the target. Display progresses smoothly through the entire
   *  motor movement at ~90% of actual speed.
   */
  if (inFeedbackCycle) {
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n=======================================================");
  Serial.println("  BOOT: POLAR ALIGNMENT CONTROLLER V14.64 (ESP32)");
  Serial.println("=======================================================\n");
